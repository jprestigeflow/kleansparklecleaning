    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function forceVideoPlay() {
            ...
            if(vid) { 
                ...
                vid.play().catch(e => console.log("Video Autoplay Issue:", e)); 
            }
        }
        ...
        gsap.utils.toArray('.reveal').forEach(elem => {
            gsap.from(elem, { scrollTrigger: { trigger: elem, start: "top 90%" }, y: 40, opacity: 0, duration: 1 });
        });
    ...
    function nextStep() {
        ...
        gsap.to(`#step-${currentStep}`, { opacity: 0, x: -30, display: 'none', onComplete: () => {
            currentStep++; const next = document.getElementById(`step-${currentStep}`);
            if(next) { next.style.display = 'block'; gsap.fromTo(next, { opacity: 0, x: 30 }, { opacity: 1, x: 0 }); }
        }});
    }
